...
unordered_set<string> legal_end_boards;

void generate_legal_end_boards() {
			...
			for (int j = 0; j < 3; ++j) {
					if (board[i][j] == '.') {
						...
						if (legal_boards.count(board_hash) == 0) {
							...
							if (terminal(board))
								legal_end_boards.insert(board_hash);
							else
								q.push({board, !x});
						}
						board[i][j] = '.';
...
int main() {
	...
	generate_legal_end_boards();
	string board = "";
	while (cin>>board && board != "end") {
		auto it = legal_end_boards.find(board);
		if (it != legal_end_boards.end())
			cout<<"valid\n";
		else
			cout<<"invalid\n";
	}
